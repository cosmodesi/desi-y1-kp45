...
def basicreader(data_file,zcosmo,Lbox,HubbleParam):
    segment_files = glob.glob(data_file+'*.fits')
...
try:
    opts, args = getopt.getopt(argv,"hi:o:n:c:z:",["help","infile=","outfile=","n_rand=","n_cpu=","zbox="])
except getopt.GetoptError:
    print('mock.py -i <infile> -o <outfile> -n <n_rand> -c <n_cpu>  -z <zbox>')
    sys.exit(2)
for opt, arg in opts:
    if opt == '-h':
        print('mock.py -i <infile> -o <outfile> -n <n_rand> -c <n_cpu> -z <zbox>')
...
LRG_path="/caefs/data/desi/mocks/cubic_mocks/LRG/z0.800/"
...
##create random
#try:
    #RR_cat=fits.open(rand_path+"cat_R"+str(n_random)+".fits",memmap=True)[1].data
    #rfile_exists=True
    #print("file there")
#except:
...
#if (rfile_exists==False):
    #n_gal=len(data)
    #n_rand=n_gal*n_random
    #read_datatype=[('x','f'),('y','f'),('z','f')]
    #RR_cat=np.zeros(n_rand,dtype=read_datatype)
    
    #rng = np.random.default_rng()
    #RR_cat['x']=rng.random(n_rand)*Lbox
    #RR_cat['y']=rng.random(n_rand)*Lbox
    #RR_cat['z']=rng.random(n_rand)*Lbox
    
    #fits.writeto(rand_path+"cat_R"+str(n_random)+".fits", RR_cat, overwrite=True)
...
result.save("samples/mockchallenge/box/abacus/"+savefile)
result.save_txt("samples/mockchallenge/box/abacus/"+savefile+".dat")
result.save_txt("samples/mockchallenge/box/abacus/"+savefile+"_poles.dat",  ells=(0, 2, 4))
print("results saved")
