...
if os.path.exists(base_path):
    pk_xi_dir_blinded = base_path
    catalogs_dir_blinded = base_path
else:
...
def get_observable_likelihood(theory_name='velocileptors', 
    ...
    if observable_name == 'power':
        data = os.path.join(pk_xi_dir_blinded, 'pk', 'pkpoles_{}_GCcomb_{}_{}_default_FKP_lin'.format(tracer,  zlim[0], zlim[1]))
        wmatrix = os.path.join(pk_xi_dir_blinded, 'pk', 'wmatrix_smooth_{}_GCcomb_{}_{}_default_FKP_lin'.format(tracer,  zlim[0], zlim[1]))
        if rpcut:
    ...
    if observable_name == 'corr':
        data = os.path.join(pk_xi_dir_blinded, 'xi','smu/allcounts_{}_GCcomb_{}_{}_default_FKP_lin_njack0_nran4_split20'.format(tracer,  zlim[0], zlim[1]))
...
if __name__ == '__main__':
    ...
    config_file_path = "/global/homes/u/uendert/desi-y1-kp45/blinding/py/config.yaml" # path to config.yaml file
